...
A：依次检查：① LSPosed Manager 中模块是否已启用；② 作用域是否勾选了**系统界面（SystemUI）**——覆盖安装或更新模块后作用域勾选可能掉落，这是失效的第一嫌疑；③ 作用域变更后需重启系统界面（或整机重启）。

**Q：修改图标后需要重启吗？**

A：不需要。规则与烘焙产物通过 FileObserver 实时监听，保存即热生效；若个别 ROM 上未生效，重启系统界面即可。

**Q：小米 / 一加等定制 ROM 上表现异常？**

A：项目以 AOSP 为主线开发，OOS（一加 15）为第一真机环境；定制 ROM 的私有行为（MIUI / HyperOS / ColorOS 等）可能造成差异，不作逐家适配承诺。

**Q：会联网吗？上传数据吗？**

A：仅在你主动同步规则库或下载图标资产时访问对应仓库；无订阅推送、无遥测、无崩溃上报、不上传任何数据。
